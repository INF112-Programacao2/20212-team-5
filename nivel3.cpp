...
funcao_cair_doce_listrado_vertical(int aux_4_, int x, int y){// 4 FAZ 1 LISTRADO, vai ser invocado no check 4
...
funcao_cairDoce_listrado_horizontal(int aux_5,int y_dir, int x_dir){ //4 FAZ 1 LISTRADO, vai ser invocado no check 4
...
funcao_cair_doce_chocolate(int tipo, int x_dir, int y_dir){ // pra ser invocada no check 5
    if(tipo==1){
        MAPA[y_dir][x_dir]=10;
        MAPA[y_dir][x_dir]=4;
        MAPA[y_dir+2][x_dir]=10;
    }
        ...
        if(check_type=6){
         ...
         if(check_type==10){
                    if(vertical){
                        ...
                        for( int w=3; x-w>=0;w++){
                            if(x+aux3>=0){
                                MAPA[Y_click_dir+aux3][X_click_dir]=MAPA[Y_click_dir-w][X_click_dir];
                                aux3--;
                            }                  
                        }  
                    ...
                    if(horizontal){                
                        for(int k=0; x-k>0; k++){
                                MAPA[Y_click_dir-k] [X_click_dir-2] = MAPA [Y_click_dir-1-k] [X_click_dir-2];
                                MAPA[Y_click_dir-k] [X_click_dir-1] = MAPA [Y_click_dir-1-k] [X_click_dir-1];
                                MAPA[Y_click_dir-k] [X_click_dir+1] = MAPA [Y_click_dir-1-k] [X_click_dir+1];
                                MAPA[Y_click_dir-k] [X_click_dir+2] = MAPA [Y_click_dir-1-k] [X_click_dir+2];
                                //MAPA[0]
...
int funcao_aux_check3_esp(int i, int j, int tipo_positivo, int tipo_listrado, int &n_doces){ //vai ser invocado na funcao check especial
...
int funcao_aux_check4_esp(int i, int j, int tipo_positivo, int tipo_listrado, int &n_doces){ //vai ser invocado na funcao check especial
...
int aux_check_especial_choc(int x_esq, int y_esq, int x_dir, int y_dir){ //check tipo de doce que vai ser trocado com o chocolate
    //VERIFICA SE É LARANJA
    if(MAPA[y_esq][x_esq]==10 && MAPA[y_dir][x_dir]==0 || (MAPA[y_esq][x_esq]==0 && MAPA[y_dir][x_dir]==0 )  ){
            for (int i=11; i<18; i++){
                for (int j=0; j<5; j++){
                    //cout<<"caralho!!!!"<<endl;
                    if(MAPA[i][j]==0){
...
int funcao_check_especial(int i, int j){
        ...
        //verifica trios e quartetos que contem uma peça especial pra cada uma das cores
...
funcao_check_5(int i, int j){
    ...
    if(MAPA[i][j]==MAPA[i+1][j] && MAPA[i][j]==MAPA[i+2][j]   &&   MAPA[i][j]==MAPA[i+3][j] && MAPA[i][j]==MAPA[i+4][j]){
        ...
        vertical=true;
        funcao_cair_doce_chocolate(1, X_click_dir,  Y_click_dir); // pra ser invocada no check 5
        funcao_cair_doces(10,i,j);
    ...
    if((MAPA[i][j]==MAPA[i][j+1] && MAPA[i][j]==MAPA[i][j+2]   &&   MAPA[i][j]==MAPA[i][j+3] && MAPA[i][j]==MAPA[i][j+4])){
        ...
        MAPA[i][j+4]=4;
        horizontal=true;
        funcao_cair_doce_chocolate(2, X_click_dir,  Y_click_dir); // pra ser invocada no check 5
        funcao_cair_doces(10,i,j);
    } 
        horizontal=false;
}

funcao_spawn_doces(){ //LOGICA COMEÇA AQUI!!!!
    for (int i=11; i<18; i++){
        for (int j=0; j<5; j++){
            int auxteste;
            ...
            if(MAPA[i][j]==10){
                 al_draw_bitmap(brigadeiroespecial,(j*q)+700,(i-11)*q,0);
            }
            funcao_check_5(i,j); 
            aux_check_especial_choc(X_click_esq,Y_click_esq,X_click_dir,Y_click_dir);
            /* funcao_check_especial(i,j); //VAI FAZER CHECKS 3 E 4 LISTRADOS!,
