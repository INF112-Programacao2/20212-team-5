...
funcao_pontuacao(){
    cout<<"azuis_total= "<<n_doces_azul<<endl;
    cout<<"roxos_total= "<<n_doces_roxo<<endl;
    cout<<"verdes_total= "<<n_doces_verde<<endl;
    
    if(n_doces_azul>=5){
        cout<<"1 objetivo conquistado!!!"<<endl;
    }
    if(n_doces_roxo>=5){
        cout<<"1 objetivo conquistado!!"<<endl;
    }
    ...
    if(n_doces_azul>=5 && n_doces_roxo>=5 && n_doces_verde>=5){
        
        cout<<"VENCEU!!!! >D >D :D ;D"<<endl;
        sair=true;
    }
}
        ...
        //VAI FA
         if(check_type=6){
            if(vertical){
                ...
                //for(int w=17; w>=11;w--){
                    MAPA[17][y]==5;
                    //cout<<"mapa["<<w<<"]["<<y<<"]= "<<"mapa["<<aux3<<"]["<<y<<"]   ("<<MAPA[aux3][y]<<")"<<endl;
                    //cout<<"mapa= "<<MAPA[w][y];
                    //aux3--;
                //}
...
funcao_check_3(int i,int j){
     ...
     if(MAPA[i][j]==MAPA[i+1][j] && MAPA[i][j]==MAPA[i+2][j]){ //verifica se a primeira posição do trio e suas posições abaixo são iguais
        ...
        if(MAPA[i][j]==1){ //se o check for verde
            n_doces_roxo=n_doces_roxo+3;

            cout<<"3pecas roxas foram pro saco!!!"<<endl;
        }
        
        
        
        if(MAPA[i][j]==2){ //se o check for roxo
            n_doces_verde=n_doces_verde+3;
            cout<<"3pecas verdes foram pro saco!!!"<<endl;
        }

        if(MAPA[i][j]==6){ //se o check for azul
            n_doces_azul=n_doces_azul+3;
            cout<<"3pecas azuis foram pro saco!!!"<<endl;
        }
    ...
    if(MAPA[i][j]==MAPA[i][j+1] && MAPA[i][j]==MAPA[i][j+2]){
        ...
        if(MAPA[i][j]==1){ //se o check for roxo
            n_doces_roxo=n_doces_roxo+3;
            cout<<"3pecas roxas foram pro saco!!!"<<endl;
        }
        ...
        if(MAPA[i][j]==2){ //se o check for verde
            n_doces_verde=n_doces_verde+3;
            cout<<"3pecas verde foram pro saco!!!"<<endl;
        }
...
funcao_check_4(int i, int j){
    if(  MAPA[i][j]==MAPA[i+1][j] && MAPA[i][j]==MAPA[i+2][j]  && MAPA[i][j]==MAPA[i+3][j] ){ 
        ...
        if(MAPA[i][j]==1){ //se o check for verde
            n_doces_roxo=n_doces_roxo+4;
            
            cout<<"4pecas roxas foram pro saco!!!"<<endl;
        }
        
        
        
        if(MAPA[i][j]==2){ //se o check for roxo
            n_doces_verde=n_doces_verde+4;
            cout<<"4pecas verdes foram pro saco!!!"<<endl;
        }

        if(MAPA[i][j]==6){ //se o check for azul
            n_doces_azul=n_doces_azul+4;
            cout<<"4pecas azuis foram pro saco!!!"<<endl;
        }
    ...
    for (int i=11; i<17; i++){
        for (int j=0; j<3; j++){
            ...
            if(MAPA[i][j]==0){ //Se for 0, desenha a peça laranja
                    al_draw_bitmap(docesimpleslaranja,(j*q)+705,(i-11)*q,0);
                    
            }
            if(MAPA[i][j]==1){ //Se for 1, desenha a peça roxa;
                    al_draw_bitmap(docesimplesroxo,(j*q)+710,(i-11)*q,0); //função desenha
            }
            if(MAPA[i][j]==2){//Se for 2, desenha a peça verde;
                    al_draw_bitmap(docesimplesverde,(j*q)+710,(i-11)*q,0); //função desenha
            
            }
            if(MAPA[i][j]==3){ //Se for 3, desenha a peça vermelha
                al_draw_bitmap(docesimplesvermelho,(j*q)+705,(i-11)*q,0);
                    
            }
            if(MAPA[i][j]==5){ //Se for 5, desenha a peça amarela;
                al_draw_bitmap(docesimplesamarelo,(j*q)+705,(i-11)*q,0); //função desenha
            }
            if(MAPA[i][j]==6){//Se for 6, desenha a peça azul;
                al_draw_bitmap(docesimplesazul,(j*q)+705,(i-11)*q,0); //função desenha
            }
            
            if(MAPA[i][j]==8){
                al_draw_bitmap(especialhorizontallaranja,(j*q)+710,(i-11)*q,0);
            }       
...
int inicializa() {
    ...
    mapa = al_load_bitmap("tabuleiroNivel1.bmp");
    if(!mapa)
    {
