...
int q = 110;  
int aux,aux2,aux3;
int definir_numero_de_movimentos_do_nivel=10; /// 
...
funcao_check_especial(int i, int j){
    
     //laranja
    if( MAPA[i][j]==8 && (MAPA[i+1][j]==0) ||  ( MAPA[i][j]==8 && MAPA[i-1][j]==0) ){
        cout<<"GORA TEM UM LARANJA E UM ESPECIAL JUNTO"<<endl;
         for(int k=18; k>11; k--){
            MAPA[k][j]=4;
        } 
        vertical=true;
        ...
        else if (ev.type== ALLEGRO_EVENT_MOUSE_BUTTON_DOWN) //se der o click no mouse
        {
            if(ev.mouse.button & 1){ // &1 = botão esquerdo
                ...
                if(pressao==true){
                   ...
                   cout<<"y_click_esq"<<Y_click_esq<<endl;
                   cout<<"X_click_esq"<<X_click_esq<<endl;
                }
            ...
            if(ev.mouse.button & 2){ // &1 = botão direito
                ...
                if(pressao_dir==true){  
                    ...
                    cout<<"y_click_dir"<<Y_click_dir<<endl;
                    cout<<"X_click_dir"<<X_click_dir<<endl;
